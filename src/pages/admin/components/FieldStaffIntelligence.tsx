  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center p-12 text-slate-500 gap-3">
        <Users className="w-6 h-6 animate-spin text-indigo-600" />
        <span className="font-medium text-sm">Loading personnel AI analytics...</span>
      </div>
    );
  }
  ...
  if (!data || data.length === 0) {
    return (
      <div className="p-12 text-center text-slate-500 bg-white rounded-xl border border-slate-200">
        ...
        <p className="font-bold text-slate-700">No Personnel Intelligence Data Available</p>
        <p className="text-xs text-slate-400 mt-1">Run an intelligence sync or inspect field staff activity.</p>
      </div>
    );
  }
